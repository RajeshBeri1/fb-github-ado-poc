...
const options = {
    ...
    from: /import \{\s(.*?)\s\} from '(.*?)'.*\s*((import \{\s.*?\s\} from '.*?'.*\s*)*)(?=export\sclass\s\1).*\{ /g,  // string or regex
    //from: /import \{\s(.*?)\s\}.*\s((import.*\s)*)(?=export\sclass\s\1).*\{ /g,  // string or regex
    to: '$3 export class $1 {', // string or fn  (fn: carrying last argument - path to replaced file)   
    //to: '$2', // string or fn  (fn: carrying last argument - path to replaced file)   
