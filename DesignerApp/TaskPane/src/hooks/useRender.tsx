...
import { MediaHierarchyDefinitionWithId, useMediaHierarchyTemplateState } from '../pages/media-hierarchy/states/MediaHierarchyState';
...
import {
    splitFlightBars
} from '../business/engine/helpers/media-hierarchy-overlap-data-helper';
...
const useRender = () => {
    ...
    const { getCurrentFlowchart } = useFlowchart();
    ...
    const loadCurrentDefinitions = useCallback(
        (
            ...
            let { def: headerDefinition, ref: headerRef } = _getDefAndRef(
                ...
                flowchart?.HeaderDefinition ?? appContext.currentHeaderTemplateDetails?.Definition
            );
            let { def: calendarDefinition, ref: calendarRef } = _getDefAndRef(
                appContext.currentCalendarTemplateDetails,
                flowchart?.CalendarDefinition ?? appContext.currentCalendarTemplateDetails?.Definition
            );
            ...
            let { def: mediaHierarchyDefinition, ref: mediaHierarchyRef } =
                _getDefAndRef(
                    ...
                    flowchart?.MediaHierarchyDefinition ?? appContext.currentMediaHierarchyDetails?.Definition
                );
            let { def: totalsDefinition, ref: totalsRef } = _getDefAndRef(
                appContext.currentTotalsDetails,
                flowchart?.TotalsDefinition ?? appContext.currentTotalsDetails?.Definition
            );
            let { def: grandTotalsDefinition, ref: grandTotalsRef } =
                _getDefAndRef(
                    appContext.currentGrandTotalsDetails,
                    flowchart?.GrandTotalDefinition ?? appContext.currentGrandTotalsDetails?.Definition
                );
            let { def: themeDefinition, ref: themeRef } = _getDefAndRef(
                appContext.currentThemeTemplateDetails,
                flowchart?.ThemeDefinition ?? appContext.currentThemeTemplateDetails?.Definition
            );
            let { def: footerDefinition } = _getDefAndRef(
                appContext.currentFooterTemplateDetails,
                flowchart?.FooterDefinition ?? appContext.currentFooterTemplateDetails?.Definition
            );
    ...
    const getAllBlocks = useCallback(
        async (
            ...
            reRenderRequired?: boolean = true,
            staticFiltering?: boolean = false,
            splitLevelIds?: string[],
        ): Promise<[allBlocks: Block[][], precisedValue: boolean, isDecimal: boolean, levelFlowchartData: API.FlowchartData]> => {
            // Init builder
            ...
            const flowchartDefinition = getCurrentFlowchart();
            ...
            let levelFlowchartData: API.FlowchartData = persistedLevelFlowchartData;
            ...
            if (clientId && calendarRef && mediaHierarchyRef) {
                if (levelFlowchartData == null) {
                    const levelApiData = await getDataCached({
                        ...
                        FlowchartDefinition: {
                            ...
                            ThemeDefinition: themeRef?.TemplateId ? themeRef : null,
                        },
                        ...
                        PreviousData: staticFiltering ? appContext.previousData : null,
                        UseStaticFilter: staticFiltering
                    });
                ...
                // save the master data for static filtering
                if (appContext.mode && !staticFiltering) {
                    appContext.setCurrentRenderedTemplateDetails(plainToClass(Definition,
                        {
                            headerDefinition: headerDefinition,
                            calendarDefinition: calendarDefinition,
                            calendarOverlayDefinition: calendarOverlayDefinition,
                            themeDefinition: themeDefinition,
                            footerDefinition: footerDefinition,
                            mediaHierarchyRef: mediaHierarchyRef,
                            ...
                            mediaHierarchyDefinition: mediaHierarchyDefinition,
                            themeRef: themeRef,
                            restrictions: restrictions ?? [] as RunRestriction[],
                            isDataLoaded: true,
                            lastRenderedDateTime: new Date()
                        })
                    );
                    appContext.setPreviousData(levelApiData?.data?.RawData);
                }
            ...
            headerDetailsBlocks = builder.move(headerDetailsBlocks, {
                left: leftSpace || logoLeftColumns,
            });
        ...
        [loadCurrentDefinitions, clientId, columns, appContext, currentLevelFlowchartData, setCurrentLevelFlowchartData]
    );
    ...
    const onWorkSheetFormatChanged = async (event: Excel.WorksheetFormatChangedEventArgs) => {
        ...
        try {
            await Excel.run(async (context) => {
                ...
                await context.sync();
                const usedRange = event.getRangeOrNullObject(sheet.context);
                await context.sync();
                ...
                if (!appContext?.onRun) {
                    const custommStylesSession = JSON.parse(sessionStorage.getItem(customStylesKey)) as Array<{ address: string, format: Excel.RangeFormat }>;
                    const currentStyles = custommStylesSession != null ? [...custommStylesSession, ...cellFormats] : [...cellFormats];
                    sessionStorage.setItem(customStylesKey, JSON.stringify(currentStyles));
                    appContext.setUnsavedChanges((prevState) => ({ ...prevState, manualFormat: true }));
                }
                else {
                    const custommStylesSession = JSON.parse(sessionStorage.getItem(runConfigCustomStylesKey)) as Array<{ address: string, format: Excel.RangeFormat }>;
                    const currentStyles = custommStylesSession != null ? [...custommStylesSession, ...cellFormats] : [...cellFormats];
                    sessionStorage.setItem(runConfigCustomStylesKey, JSON.stringify(currentStyles));
                }
    ...
    const render = useCallback(
        async (definitionName?: TDefinitionName, definition?: TDefinition, restrictions?: RunRestriction[]) => {
            showWarning(definitionName, definition);
            ...
            setTimeout(async () => {
                const value = getStorageKey('isRendering');
                const renderCheck = isRerenderRequired(definitionName, definition, restrictions, appContext.mode);
                if (isRendering || (value && value == "true") || (appContext.mode ? (!renderCheck.rerenderRequired && !renderCheck.staticFiltering) : false)) return;
                setStorageKey('isRendering', true);
                flushSync(() => {
                    setIsRendering(true);
                ...
                try {
                    ...
                    blocksData = await getAllBlocks(
                        ...
                        definition,
                        restrictions,
                        renderCheck.rerenderRequired,
                        renderCheck.staticFiltering
                    );
                    ...
                    if (themeDefinition?.Styling?.Position?.Start) {
                        const positions = themeDefinition.Styling.Position.Start.split(/^([A-Z]+)([0-9]+)$/);
                    ...
                    // ensure this flag is defined for later logic regardless of branch
                        ...
                        if (!appContext.initialRenderDone) {
                            ...
                            if (!isEmpty(sessionStorage.getItem(customStylesKey))) {
                                if (!appContext.isRenderClicked) {
                                    ...
                                    appContext.setIsRenderClicked(true);
                                }
                    ...
                    setStorageKey('isRendering', false);
        ...
        [getAllBlocks, loadCurrentDefinitions, isRendering, appContext, setIsGlobalRendering, pushNotification]
    );
    useEffect(() => {
        setPersistedLevelFlowchartData(null);
    }, [render])
    useEffect(() => {
        if (
            // commented as it was loading with old template details for run mode--> uncommented as we have to set initialRenderDone as true
            (appContext.mode && !isEmpty(appContext.flowchartRunConfiguration) &&
                appContext.flowchartTemplateDefinition?.Definition &&
                appContext.flowchartTemplateDefinition?.OmniClientId &&
                !appContext.initialRenderDone
                && appContext.columns && appContext.columns.length
            ) ||
            (!appContext.mode &&
                appContext.flowchartTemplateDefinition?.Definition &&
                appContext.flowchartTemplateDefinition?.OmniClientId &&
            ...
            setStorageKey('isRendering', false);
            // Guard against multiple components starting initial render concurrently
            if (!appContext.initialRenderStarted) {
                appContext.setInitialRenderStarted(true);
                render()
                    .then(() => {
                        appContext.setInitialRenderDone(true);
                    })
                    .catch(() => {
                        // keep initialRenderDone false on failure
                    })
                    .finally(() => {
                        appContext.setInitialRenderStarted(false);
                    });
            }
        }

    }, [appContext, render, appContext.flowchartRunConfiguration]);
    ...
    const isRerenderRequired = (definitionName?: TDefinitionName, definition?: TDefinition, restrictions?: RunRestriction[], isRun?: boolean): { rerenderRequired: boolean, staticFiltering: boolean } => {

        if (!isRun)
            return {
                rerenderRequired: true,
                staticFiltering: false,
            }
        ...
        const oldLoadedData = appContext?.currentRenderedTemplateDetails;
        if (oldLoadedData?.isDataLoaded) {
            result.rerenderRequired = true;// please change the seconds as per need
            result.staticFiltering = appContext.staticFilter && isRestrictionFilterPartOfMediaHierarchy(mediaHierarchyDefinition, restrictions, oldLoadedData.mediaHierarchyDefinition);
        }
        console.log(`rerenderRequired is - ${result.rerenderRequired}, staticFiltering is ${result.staticFiltering}`);
        return result;
    }
    const isRestrictionFilterPartOfMediaHierarchy = (mediaHierarchy: API.MediaHierarchyTemplateDetailsDTO,
        restrictions?: RunRestriction[],
        oldMediaHierarchy: API.MediaHierarchyTemplateDetailsDTO): boolean => {
        if ((!restrictions?.length || !mediaHierarchy?.Levels?.length) && oldMediaHierarchy?.Levels?.length) {
            return true;
        } else {
            const levels = mediaHierarchy.Levels.map(c => `${c.ColumnName}_${c.TableId}`);
            const filters = restrictions.map(c => `${c.ColumnName}_${c.TableId}`);
            const oldLevels = oldMediaHierarchy.Levels.map(c => `${c.ColumnName}_${c.TableId}`);
            const isLevelsSame = levels.every(c => oldLevels.includes(c));
            if (!isLevelsSame) {
                return false;
            }
            const isRestrictionsSame = filters.every(c => levels.includes(c));
            if (!isRestrictionsSame) {
                return false;
            }
            const currentMediaHierarchyLevelSettings = mediaHierarchy.Levels.map(c => {
                return {
                    Key: `${c.ColumnName}_${c.TableId}`, Settings: c.Settings.filter(s => s.Enabled).map(s => s.Name)
                }
            });
            const oldMediaHierarchyLevelSettings = oldMediaHierarchy.Levels.map(c => {
                return {
                    Key: `${c.ColumnName}_${c.TableId}`, Settings: c.Settings.filter(s => s.Enabled).map(s => s.Name)
                }
            });
            const isSettingsSame = currentMediaHierarchyLevelSettings.every(c => {
                const oldSettings = oldMediaHierarchyLevelSettings.find(o => o.Key === c.Key);
                if (oldSettings) {
                    return c.Settings.every(d => oldSettings.Settings.includes(d));
                }
                return false;
            });
    ...
    return { isRendering, render, getAllBlocks };
};
