...
// Define Option type for clarity
type Option = { id: string; value: string; disabled?: boolean };
...
export const SelectWithCommonFields: React.FC<ISelectProps> = ({
    isLoading = false,
    ...
    const commonColumns = filter(columns, ({ IsMetric, IsCommon, Name }) =>
        (IsMetric === isMetricSelection ||
            (includedMetrics.length > 0 && includedMetrics.includes(Name.toLowerCase()))) &&
        IsCommon === true
    );

    const otherColumns = filter(columns, ({ IsMetric, IsCommon, Name }) =>
        (IsMetric === isMetricSelection ||
            (includedMetrics.length > 0 && includedMetrics.includes(Name.toLowerCase()))) &&
        IsCommon === false
    );
